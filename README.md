...
├── README.md             # This file
├── LICENSE               # MIT license
├── .nojekyll             # Tells GitHub Pages to skip Jekyll (prevents build failures)
...
1. Push this repository to GitHub (e.g. `kamelaggoun/pro.portfolio`).
2. Go to **Settings → Pages**.
3. Under **Build and deployment → Source**, choose **Deploy from a branch**.
4. Select the branch (e.g. `main`) and the root folder (`/`), then **Save**.
5. The site will be published at `https://kamelaggoun.github.io/pro.portfolio/`.

### If the build fails ("pages-build-deployment → build failed")

GitHub Pages runs **Jekyll** over your site by default, and Jekyll can fail on a
plain static site like this one. This repository already includes a **`.nojekyll`**
file at the root, which tells GitHub Pages to skip Jekyll and serve the files
as-is. If you ever see a failed build:

1. Make sure an empty (or commented) file named `.nojekyll` exists at the root of
   the repository — it must be **committed and pushed** (an untracked local file
   won't help).
2. Confirm the file is spelled exactly `.nojekyll` (no extension, leading dot).
3. Re-run the workflow from the **Actions** tab, or push a new commit.

> Because the site is fully static with relative paths, it also works on any
> project pages URL without changes. Keep `sitemap.xml` and the Open Graph URLs
> in `index.html` in sync with the final URL.
